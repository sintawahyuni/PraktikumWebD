<?php
session_start();
$id = $_SESSION['id'];
$nama = $_SESSION['nama'];
$peran =  $_SESSION['peran'];

if(empty($_SESSION['id'])){
    echo"<script>alert('Mohon Login Terlebih Dahulu'); location.href='login.php'</script>";
}
if(($_SESSION['peran'])<>"mahasiswa"){
    echo"<script>alert('Anda Tidak Bisa Mengakses Halaman Ini'); location.href='login.php'</script>";
}
?>
 <html lang="en">
 <head>
     <meta charset="UTF-8">
     <title>Universitas Udayana</title>
     <link rel="stylesheet" type="text/css" href="style.css">
 </head>
 <body>
     <div class="menu">
         <img src="gambar/unud.png" class="logo">
         <div class="tag">
             <p style="font-size: larger;"><b>UNIVERSITAS UDAYANA</b><br>
             <i>Unggul, Mandiri, Berbudaya</i></p>
         </div>
         <table cellpadding="20px" class="tableMenu">
             <tr>
                 <td><a href="indexMahasiswa.php">Beranda</a></td>
                 <td><a href="tentang.php">Tentang Kampus</a></td>
                 <td><a href="profil.php">Profil Pengajar</a></td>
                 <td><a href="kontak.php">Kontak</a></td>
                 <td><a href="index.php">Logout</a></td>
             </tr>
         </table>
     </div>
     <div class="boxPic">
         <img src="gambar/kampus.jpg" class="pic">
    </div>
    <div class="boxAbout">
        <p><h2 style="color: black;font-size: 30px; text-align: center;">Tentang Kampus</h2></p>
        <div class="boxVisi">
            <h2 style="text-align: center;">Visi</h2><hr>
            <p style="text-align: center;">Terwujudnya perguruan tinggi yang unggul, mandiri, dan berbudaya.</p>
        </div>
        <div class="boxMisi">
            <h2 style="text-align: center;">Misi</h2><hr>
            <p>
                <ol>
                    <li>Menyelenggarakan pendidikan tinggi yang bermutu dan menghasilkan lulusan yang memiliki moral/etika/akhlak dan integritas yang tinggi sesuai dengan tuntutan masyarakat lokal, nasional, dan internasional;</li>
                    <li>Mengembangkan penelitian dan pengabdian kepada masyarakat sesuai dengan kepentingan masyarakat dan bangsa;</li>
                    <li>Memberdayakan Unud sebagai lembaga yang menghasilkan dan mengembangkan pengetahuan, teknologi, dan budaya yang dapat dimanfaatkan untuk kesejahteraan masyarakat; dan </li>
                    <li>Menghasilkan karya inovatif dan prospektif bagi kemajuan Unud serta perekonomian nasional. </li>
                </ol>
            </p>
        </div>
    </div>
    <div class="boxProfil">
        <p><h2 style="color: black;font-size: 30px; text-align: center;">Profil Pengajar</h2></p>
        <div class="boxFotoProfil">
            <div class="boxEachF" style="float: left; margin-left: 120px;">
                <img src="gambar/ava1.png" class="boxEachF">
            </div>
            <div class="boxEachF" style="float: left; margin-left: 150px;">
                <img src="gambar/ava3.png" class="boxEachF">
            </div>
            <div class="boxEachF" style="float: left; margin-left: 180px;">
                <img src="gambar/ava1.png" class="boxEachF">
            </div>
            <div class="boxEachF" style="float: right; margin-right: 150px;">
                <img src="gambar/ava3.png" class="boxEachF">
            </div>
            <div class="boxEachF" style="float: right; margin-right: 120px;">
                <img src="gambar/ava1.png" class="boxEachF">
            </div>
        </div>
        <div class="boxDes" style="float: left; margin-left: 60px;">
            <center>
                Budi Doremi
                <br>12345678987654234   
                <br>budidoremi@cs.unud.ac.id
                <br>Jl.Ahmad Dahlan 1 No.4
            </center>
        </div>
        <div class="boxDes" style="float: left; margin-left: 50px;">
            <center>
                Nyoman Parwati Sari Murni
                <br>12345678987654243   
                <br>sarimurni@cs.unud.ac.id
                <br>Jl.Ahmad Dahlan 1 No.9
            </center>
        </div>
        <div class="boxDes" style="float: left; margin-left: 60px;">
            <center>
                I Gede Mawang Putra 
                <br>12345678987654299   
                <br>putramawang@cs.unud.ac.id
                <br>Jl.Ahmad Dahlan 9 No.4
            </center>
        </div>
        <div class="boxDes" style="float: left; margin-left: 70px;">
            <center>
                Tomingse Rahman Putra 
                <br>12345678987654277   
                <br>tomingse@cs.unud.ac.id
                <br>Jl.Ahmad Dahlan 99 No.4
            </center>
        </div>
        <div class="boxDes" style="float: left; margin-left: 30px;">
            <center>
                Dewi Rahmawati
                <br>12345678987688234   
                <br>dewdewi@cs.unud.ac.id
                <br>Jl.Ahmad Yani 1 No.4
            </center>
        </div>
    </div>
    <div class="boxKontak">
        <p><h2 style="color: black;font-size: 30px; text-align: center;">Kontak</h2></p>
        <table align="center" cellpadding="8px">
            <tr>
                <td><img src="gambar/peraon.png" class="icon"></td>
                <td>Ni Made Sinta Wahyuni</td>
            
                <td><img src="gambar/mail.png" class="icon"></td>
                <td>madesintawahyuni@gmail.com</td>
        
                <td><img src="gambar/wa1.png" class="icon"></td>
                <td>+628199999999</td>
        
                <td><img src="gambar/line1.png" class="icon"></td>
                <td>sintawahyunii</td>
            </tr>
        </table>
    </div>
    <div class="copy">
        <p style="color: white;">Copyright 2020 || Universitas Udayana</p>
    </div>
 </body>
 </html>